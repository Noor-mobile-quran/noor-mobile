  ...
  const jd =
    Math.floor(
      (1461 *
        (today.getFullYear() +
          4800 +
          Math.floor((today.getMonth() + 1 - 14) / 12))) /
        4,
    ) +
    Math.floor(
      (367 *
        (today.getMonth() +
          1 -
          2 -
          12 * Math.floor((today.getMonth() + 1 - 14) / 12))) /
        12,
    ) -
    Math.floor(
      (3 *
...
export default function HomePage() {
  ...
  return (
    ...
    >
      ...
      <LinearGradient
        colors={
          isDark ? [colors.surface, colors.bg] : ["#4A3F30", colors.textPrimary]
        }
      ...
      >
        ...
        <View className="absolute bottom-0 left-0 right-0 h-12 opacity-5 flex-row justify-center items-end gap-8">
          <View
            className="w-1 h-10 rounded-t-full"
          ...
          />
          <View
            className="w-1 h-8 rounded-t-full"
            style={{ backgroundColor: "#FFF9ED" }}
          />
          <View
